		...
		switch(strength)
			if(0)
				if(src.safety == 0)
					...
					A = new/obj/effect/effect/sparks(src.loc)
			if(1)
				if(src.safety == 0)
...
/obj/machinery/sunreactor/center/blob_act(severity)
	return
...
/obj/machinery/sunreactor/center/proc/reactor_rad()
	...
	sleep(100)
	command_alert("Reactor core destabilized. High levels of radiation detected. Please report to the Med-bay if you feel strange.", "Radiation Alert")
...
/obj/machinery/sunreactor/center/proc/expand(var/force_size = 0)
	...
	switch(temp_allowed_size)
		...
		if(5)
			...
			command_alert("Warning! Near-critical temperature registered in reactor core!", "Engineering section alert")
		...
		if(7)
			...
			command_alert("Warning! Critical temperature registered in reactor core!", "Engineering section alert")
			for(var/mob/M in player_list)
