...
datum/preferences
	...
	var/lip_style = null				//Lips color
	var/trait = "None"					//Trait
...
/*
	var/job_starwars_high = 0
	var/job_starwars_med = 0
	var/job_starwars_low = 0
*/
...
/datum/preferences
	...
	proc/ShowChoices(mob/user)
		...
		dat += "Trait: <a href='?_src_=prefs;preference=trait;task=input'><b>Select trait.. [trait]</b></a><br>"
		...
		dat += "<br><b>Lips</b><br>"
		dat += "<a href='?_src_=prefs;preference=lips;task=input'>Change Color</a> <font face='fixedsys' size='3' color='[lip_style]'><table  style='display:inline;' bgcolor='[lip_style]'><tr><td>__</td></tr></table></font>"
...
/*
			if(STARWARS)
				switch(level)
					if(1)
						return job_starwars_high
					if(2)
						return job_starwars_med
					if(3)
						return job_starwars_low
*/
			if(CIVILIAN)
				switch(level)
					if(1)
	...
	proc/SetJobDepartment(var/datum/job/job, var/level)
		...
		switch(level)
			...
			//	job_starwars_med |= job_starwars_high
				job_civilian_high = 0
				job_medsci_high = 0
				job_engsec_high = 0
			//	job_starwars_high = 0
...
/*
			if(STARWARS)
				switch(level)
					if(2)
						job_starwars_high = job.flag
						job_starwars_med &= ~job.flag
					if(3)
						job_starwars_med |= job.flag
						job_starwars_low &= ~job.flag
					else
						job_starwars_low |= job.flag
*/
			if(CIVILIAN)
		...
		switch(href_list["task"])
			...
			if("input")
				switch(href_list["preference"])
					...
					if("lips")
						lip_style = input(user, "Choose your character's Lips colour:", "Character Preference") as color|null
					...
					if("trait")
						var/new_trait = input(user, "Choose your trait.", "Character Preference")  as null|anything in list("Kleptomaniac","None")
						if(new_trait)
							trait = new_trait
						else
							trait = "None"
